...
function main() {
	...
	for (var paragraph_index = 0; paragraph_index < paragraphs.length; paragraph_index++) {
		attempt(function() {
			...
			writeCSVPayment(payment_output, error_hit, "Error Identification", paragraph_index);
			writeCSVWait(lag_output, error_hit, "Error Identification", paragraph_index);
			
			var finalPatches = {
				sentences: paragraph,
				patches: []
			};
			findTime = getHITEndTime(error_hit) - getHITStartTime(error_hit);
			...
			for (var i=0; i<errors.length; i++) {
				...
				attempt( function() {
					...
					var outPatch = outputVotes(output, lag_output, payment_output, reason_votes, fix_votes, fix_hit, vote_hit, error, reasons, corrections, paragraph, paragraph_index);					
...
function outputVotes(output, lag_output, payment_output, reason_votes, fix_votes, edit_hit, vote_hit, patch, reasons, corrections, paragraph, paragraph_index) {
	...
	print("TODO: turn back on vote filtering. we're done with the study now.");
