...
function main() {
	...
	for (var paragraph_index = 0; paragraph_index < paragraphs.length; paragraph_index++) {
		...
		attempt(function() {
			...
			if (find) {
				...
				writeCSVPayment(payment_output, cut_hit, "Find");
				writeCSVWait(lag_output, cut_hit, "Find");
			}
			...
			for (var i=0; i<cuts.length; i++) {
				...
				attempt(function() {
					...
					var outPatch = outputEdits(output, lag_output, payment_output, paragraph, cut, cut_hit, edit_hit, vote_hit, grammar_votes, meaning_votes, suggestions);
	...
	var patchesOutput = new java.io.FileWriter("cut_patches.json");	
...
function requestPatches(paragraph) {
	...
	var webpage = s3.putString(slurp("../template/identify-patches.html").replace(/___PARAGRAPH___/g, text).replace(/___ESCAPED_PARAGRAPH___/g, escape(text)));
...
function requestEdits(cut) {	
	...
	var webpage = s3.putString(slurp("../template/cut.html").replace(/___TEXT___/g, full_text)
...
function joinEdits(edit_hit, originalSentence, paragraph_index) {
	...
	var hit = mturk.boundedWaitForHIT(hitId, wait_time, 1, edit_redundancy);
	print("done! completed by " + hit.assignments.length + " turkers");
...
function requestVotes(cut, options, edit_hit) {		
	...
	var webpage = s3.putString(slurp("../template/vote_errors.html")
...
function joinVotes(vote_hit, paragraph_index) {
	...
	var hit = mturk.boundedWaitForHIT(hitId, wait_time, 1, verify_redundancy);
	print("done! completed by " + hit.assignments.length + " turkers");
...
function outputEdits(output, lag_output, payment_output, paragraph, cut, cut_hit, edit_hit, vote_hit, grammar_votes, meaning_votes, suggestions)
{	
